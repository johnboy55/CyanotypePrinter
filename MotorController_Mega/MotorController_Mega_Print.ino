/* MotorController_Mega_Print.ino
 * Printhead and BMP routines for the Mega sketch.
 */
...
void printEntireBMP(const String &path) {
  ...
  display.setCursor(0, 28); displayPrintf(display,"W=%d  H=%d", meta.W, meta.H);
  ...
  for (int slice = 0; slice < totalSlices; ++slice) {
    ...
    if (!readFourRowsAt(path, meta, visualStart, img)) {
      oledHeader("BMP");
      display.setCursor(0, 20); display.print("Read error at slice ");
    ...
    display.setCursor(0, 30); displayPrintf(display,"Slice %d/%d", slice+1, totalSlices);
    ...
    if (slice < totalSlices - 1) {
      ...
      char buf[24]; snprintf(buf, sizeof(buf), "Y +%.2fmm", CFG.yBandAdvanceMM);
...
void printEntireBMP_4by(const String &path) {
  ...
  for (int slice = 0; slice < totalSlices; ++slice) {
    ...
    if (slice < totalSlices - 1) {
      bool none[4] = {false,false,false,false};
      char buf[24]; snprintf(buf, sizeof(buf), "Y +%.2fmm", CFG.mmPerPixel);
      drawStatusRow(none, String(buf));
